...
### Removed（削除）
- **フレーズタブ／トランスクリプト表示エリアに起動時から表示していたサンプルデータ**（`js/01-data.js` に埋め込まれていたデモ用の英文・和訳・発音アノテーション）を削除し、`PARAS` は起動時は空配列に。プロジェクト読込前は両エリアとも空欄になる。`totalDur` の初期値も `PARAS` が空の場合は `0` にフォールバック。
