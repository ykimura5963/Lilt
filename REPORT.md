# Lilt — プロジェクト分析レポート
*2026-06-07 時点 / アプリバージョン `LILT_VERSION = 1.4.1`*
...
- ☆**日本語訳のみ再生成（🌐訳）** — 保存済みプロジェクトの英文を、DL・字幕取得・注釈をやり直さず翻訳して `ja` を補完（`POST /retranslate/{id}`、SSE進捗、RunPodコールドスタート用のウィンドウ単位リトライ付き）。**プロバイダ（Ollama/RunPod/OpenRouter/OpenAI）を一覧上で都度選択可**。RunPodタイムアウトで英語のみになった既存データの復旧に使用
